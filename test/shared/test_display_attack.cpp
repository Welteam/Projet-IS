...
{
    {
        ...
        vector<int> field = DisplayAttack::createField(unit.get(), gameState.getWorld());
        BOOST_CHECK_EQUAL(field.size(), 400);
        unit.get()->levelUp();
        vector<int> field1 = DisplayAttack::createField(unit.get(), gameState.getWorld());
        BOOST_CHECK_EQUAL(field1.size(), 400);
        unit.get()->levelUp();
        vector<int> field2 = DisplayAttack::createField(unit.get(), gameState.getWorld());
        BOOST_CHECK_EQUAL(field2.size(), 400);
        unit.get()->levelUp();
        vector<int> field3 = DisplayAttack::createField(unit.get(), gameState.getWorld());
