...
public:
    ...
    static bool isValid(int x, int y, const state::World &world,
                        const vector<state::GameObject> &gameObjects) { //If our Node is an obstacle it is not valid
        int id = x + y * 20;
    ...
    static vector<Node> aStar(Node player, Node dest, const state::World &world,
                              const vector<state::GameObject> &gameObjectsWithUnitSelected, int pm) {
        vector<Node> empty;
    ...
    static vector<Node> aStar(Node player, Node dest, const state::World &world,
                              const vector<state::GameObject> &gameObjectsWithUnitSelected) {
        vector<Node> empty;
        empty.push_back(player);
        vector<state::GameObject> gameObjects;
        for (auto gameObject : gameObjectsWithUnitSelected) {
            if (gameObject.getX() != player.x || gameObject.getY() != player.y){
                if (gameObject.getX() != dest.x || gameObject.getY() != dest.y){
                    gameObjects.push_back(gameObject);
                }
            }
